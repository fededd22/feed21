...
// -----------------------------------------------------------------------------
// SSH (Dropbear) protocol
// -----------------------------------------------------------------------------
// Runs alongside VLESS/VMess/Trojan: Dropbear only ever listens on a local
// port (never exposed directly), and the client reaches it exactly the same
// way as the other protocols -- a WebSocket connection to this same public
// domain on port 443, TLS-terminated by the hosting platform, with the same
// fake/spoofable SNI. See the "SSH" branch of the `upgrade` handler below for
// the actual bridge, which pipes raw bytes (no WS framing) between the client
// socket and Dropbear -- Dropbear itself handles username/password auth, so
// there's no per-client UUID system here, just the one shared OS account.
const DROPBEAR_PORT = 10093;
const SSH_WS_PATH = "/by_moon_ssh";
let dropbearProcess: ChildProcess | null = null;
let dropbearRestartAttempts = 0;
...
function getSshCredentials(): SshCredentials {
  ...
  return {
    username: process.env.SSH_USERNAME || "vpsuser",
    password: process.env.SSH_PASSWORD || "vpspass"
  };
}
...
function findNologinShell(): string {
  for (const candidate of ["/usr/sbin/nologin", "/sbin/nologin", "/usr/bin/nologin", "/bin/false"]) {
    if (fs.existsSync(candidate)) return candidate;
  }
  return "/bin/sh";
}

// Creates (or, on a username change, renames) the Linux account Dropbear
// authenticates against, and (re)sets its password -- callable both at
// startup and live from the bot when an admin changes the SSH
// username/password, so a change takes effect immediately without a
// redeploy. Runs as root (this container has no USER directive), same as
// the old shell-script version, but centralized here so it's logged
// through addLog (visible in "📝 سجلات الخادم" in the bot) instead of only
// in the container's raw stdout.
function ensureSshSystemAccount(creds: SshCredentials, previousUsername?: string): boolean {
  try {
    const shell = findNologinShell();

    // Dropbear rejects logins outright if the account's shell isn't listed
    // in /etc/shells -- see the incident this comment is protecting against.
    const shellsPath = "/etc/shells";
    let shells = "";
    try { shells = fs.readFileSync(shellsPath, "utf8"); } catch { /* file may not exist yet */ }
    if (!shells.split("\n").map(s => s.trim()).includes(shell)) {
      fs.appendFileSync(shellsPath, `${shell}\n`);
      addLog(`[SSH] Added '${shell}' to /etc/shells.`);
    }

    const userExists = (name: string) => {
      try { execSync(`id ${name}`, { stdio: "ignore" }); return true; } catch { return false; }
    };

    if (previousUsername && previousUsername !== creds.username && userExists(previousUsername)) {
      execSync(`usermod -l ${creds.username} ${previousUsername}`, { stdio: "ignore" });
      addLog(`[SSH] Renamed SSH user '${previousUsername}' -> '${creds.username}'.`);
    } else if (!userExists(creds.username)) {
      execSync(`useradd -m -s ${shell} ${creds.username}`, { stdio: "ignore" });
      addLog(`[SSH] Created SSH user '${creds.username}'.`);
    }

    execSync(`echo '${creds.username}:${creds.password.replace(/'/g, "'\\''")}' | chpasswd`, { stdio: "ignore" });
    addLog(`[SSH] Password set for SSH user '${creds.username}'.`);
    return true;
  } catch (err: any) {
    addLog(`[SSH] ERROR applying system account for '${creds.username}': ${err?.message || err}`);
    return false;
  }
}

function startDropbear() {
  const dropbearBin = "/usr/sbin/dropbear";
  if (!fs.existsSync(dropbearBin)) {
    addLog("[SSH] dropbear binary not found -- SSH protocol disabled (is it installed in this image?).");
    return;
  }

  ensureSshSystemAccount(getSshCredentials());

  const keyDir = process.env.DROPBEAR_KEY_DIR || path.join(process.env.DATA_DIR || "/app/data", "dropbear");
  const rsaKey = path.join(keyDir, "dropbear_rsa_host_key");
  const ecdsaKey = path.join(keyDir, "dropbear_ecdsa_host_key");

  const args = ["-F", "-E", "-p", `127.0.0.1:${DROPBEAR_PORT}`, "-w", "-K", "30"]; // -w: no root login, -K: send SSH keepalive every 30s
  if (fs.existsSync(rsaKey)) args.push("-r", rsaKey);
  if (fs.existsSync(ecdsaKey)) args.push("-r", ecdsaKey);

  addLog(`[SSH] Starting Dropbear on 127.0.0.1:${DROPBEAR_PORT}...`);
  const child = spawn(dropbearBin, args, { env: process.env });

  child.stdout?.on("data", (d) => { const m = d.toString().trim(); if (m) addLog(`[SSH] ${m}`); });
  child.stderr?.on("data", (d) => { const m = d.toString().trim(); if (m) addLog(`[SSH] ${m}`); });

  child.on("exit", (code) => {
    addLog(`[SSH] Dropbear exited with code ${code}.`);
    dropbearProcess = null;
    // Self-healing: same idea as the webhook retry loop -- if Dropbear dies
    // (crash, OOM, etc.) restart it automatically with a short backoff
    // instead of leaving the SSH protocol silently dead until a manual
    // redeploy.
    dropbearRestartAttempts++;
    const delayMs = Math.min(30000, 2000 * dropbearRestartAttempts);
    setTimeout(startDropbear, delayMs);
  });

  child.on("spawn", () => {
    dropbearRestartAttempts = 0;
  });

  dropbearProcess = child;
}

async function sendSshInfo(chatId: number | string) {
  const domain = getPublicDomain();
  const creds = getSshCredentials();
  const running = !!dropbearProcess;

  const text =
    `🔐 <b>SSH (Dropbear) — SSL / WebSocket على المنفذ 443</b>\n\n` +
    `${running ? "🟢 يعمل الآن" : "🔴 متوقف حاليًا"}\n\n` +
    `🌐 <b>الهوست (Host / SNI):</b> <code>${escapeHtml(domain)}</code>\n` +
    `🔌 <b>المنفذ:</b> <code>443</code>\n` +
    `🧭 <b>المسار (Path):</b> <code>${SSH_WS_PATH}</code>\n` +
    `🕵️ <b>SNI مموّه:</b> <code>${FAKE_SNI_HOST}</code> (يمكنك استخدامه بدل نطاق الخادم إذا كانت الشبكة تراقب حسب اسم النطاق)\n` +
    `👤 <b>Username:</b> <code>${escapeHtml(creds.username)}</code>\n` +
    `🔑 <b>Password:</b> <code>${escapeHtml(creds.password)}</code>\n\n` +
    `📱 استخدم أي تطبيق يدعم "SSH over WebSocket/Payload" (مثل HTTP Injector أو NPV Tunnel/DarkTunnel)، بروتوكول TLS/SSL، بنفس الهوست والمسار والمنفذ أعلاه.\n\n` +
    `⚠️ هذا حساب مشترك واحد لكل من يملك المسار — غيّره من الأزرار أدناه قبل نشره لأي طرف لا تثق به.`;
  ...
  await telegramApi("sendMessage", {
    ...
    reply_markup: {
      inline_keyboard: [
        [{ text: "👤 تغيير اسم المستخدم", callback_data: "ssh_change_username" }],
        [{ text: "🔑 تغيير كلمة المرور", callback_data: "ssh_change_password" }]
      ]
    }
  });
  await telegramApi("sendMessage", { chat_id: chatId, text: "⬆️", reply_markup: MAIN_REPLY_KEYBOARD });
}
...
async function handleTelegramUpdate(update: any) {
  ...
  if (update.message) {
    ...
    if (text === "/ssh" || text === "🔐 SSH" || text === "SSH" || textLower === "ssh") {
    ...
    // SSH username/password change (applied live, no redeploy needed)
    if (session.action === "ssh_change_username") {
      const newUsername = text.trim();
      if (!/^[a-zA-Z_][a-zA-Z0-9_-]{2,31}$/.test(newUsername)) {
        await telegramApi("sendMessage", {
          chat_id: chatId,
          text: "❌ <b>اسم مستخدم غير صالح.</b>\nأحرف/أرقام إنجليزية فقط، يبدأ بحرف، بدون مسافات (3-32 حرفًا). أعد المحاولة:",
          parse_mode: "HTML"
        });
        return;
      }
      delete userSessions[chatId];
      const oldCreds = getSshCredentials();
      const newCreds: SshCredentials = { username: newUsername, password: oldCreds.password };
      const ok = ensureSshSystemAccount(newCreds, oldCreds.username);
      if (ok) saveSshCredentials(newCreds);
      await telegramApi("sendMessage", {
        chat_id: chatId,
        text: ok
          ? `✅ <b>تم تغيير اسم مستخدم SSH إلى</b> <code>${escapeHtml(newUsername)}</code> <b>بنجاح.</b> التغيير فعّال الآن مباشرة.`
          : "❌ <b>فشل تغيير اسم المستخدم.</b> راجع سجلات الخادم لمعرفة السبب.",
        parse_mode: "HTML"
      });
      await sendSshInfo(chatId);
      return;
    ...
    if (session.action === "ssh_change_password") {
      ...
      if (newPassword.length < 4 || /\s/.test(newPassword)) {
        await telegramApi("sendMessage", {
          chat_id: chatId,
          text: "❌ <b>كلمة مرور غير صالحة.</b>\n4 أحرف على الأقل وبدون مسافات. أعد المحاولة:",
          parse_mode: "HTML"
        });
        return;
      }
      delete userSessions[chatId];
      const oldCreds = getSshCredentials();
      const newCreds: SshCredentials = { username: oldCreds.username, password: newPassword };
      const ok = ensureSshSystemAccount(newCreds);
      if (ok) saveSshCredentials(newCreds);
      await telegramApi("sendMessage", {
        chat_id: chatId,
        text: ok
          ? "✅ <b>تم تغيير كلمة مرور SSH بنجاح.</b> التغيير فعّال الآن مباشرة."
          : "❌ <b>فشل تغيير كلمة المرور.</b> راجع سجلات الخادم لمعرفة السبب.",
        parse_mode: "HTML"
      });
      await sendSshInfo(chatId);
      return;
...
server.on("upgrade", (req, socket, head) => {
  ...
  // SSH (Dropbear) bridge: unlike VLESS/VMess/Trojan, Dropbear speaks raw
  // SSH, not WebSocket -- so instead of proxy.ws() (WS<->WS to xray) this
  // completes the HTTP upgrade by hand, then splices the socket directly
  // to a plain TCP connection to Dropbear and forwards bytes as-is in both
  // directions. Auth (username/password) happens inside the SSH handshake
  // itself, handled entirely by Dropbear -- this bridge never sees or
  // checks credentials, it's just a dumb pipe.
  if (pathname === SSH_WS_PATH) {
    const tcpSocket = net.connect({ host: "127.0.0.1", port: DROPBEAR_PORT }, () => {
      ...
      if (head && head.length) tcpSocket.write(head);
      socket.pipe(tcpSocket);
      tcpSocket.pipe(socket);
    });

    try { (socket as any).setNoDelay?.(true); } catch { /* ignore */ }
    try { tcpSocket.setNoDelay(true); } catch { /* ignore */ }
    // TCP keepalive on both legs of the bridge: without this, an idle SSH
    // session (no data for a while -- normal for an open shell/tunnel that
    // isn't actively transferring anything) can get silently dropped by an
    // intermediate NAT/load balancer that reclaims "idle" connections, which
    // looks to the client exactly like a clean disconnect + auto-reconnect.
    try { (socket as any).setKeepAlive?.(true, 20000); } catch { /* ignore */ }
    try { tcpSocket.setKeepAlive(true, 20000); } catch { /* ignore */ }

    const destroyBoth = () => { socket.destroy(); tcpSocket.destroy(); };
    tcpSocket.on("error", (err) => { addLog(`[SSH] Bridge error: ${err.message}`); destroyBoth(); });
