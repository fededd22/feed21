...
RUN apt-get update \
    && apt-get install -y --no-install-recommends curl unzip ca-certificates dropbear-bin passwd \
...
# 10093 = Dropbear (SSH), lui aussi uniquement local -- exposé au client
# final via le même tunnel WebSocket/TLS:443 que les autres protocoles
# (voir le bridge SSH dans le handler `upgrade` de server.ts), jamais en
# TCP brut directement.
EXPOSE 3000 10080 10081 10082 10090 10091 10092 10093
